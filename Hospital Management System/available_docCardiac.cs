...
namespace Hospital_Management_System
{
    public partial class available_docCardiac : Form
    {
        ...
        public void loadform()
        {
            ...
            con.Open();
            string querry = "select * from Appointment where doctor_id=" + cardiac.doctor_id;
        ...
        private void submit_Click(object sender, EventArgs e)
        {
            ...
            SqlConnection con = new SqlConnection(cs);
            con.Open();
            string querry = "select * from Appointment where doctor_id=" + cardiac.doctor_id + " and time_from=@tf and time_to=@tt and appointment_date=@ad";
            SqlCommand cmd = new SqlCommand(querry, con);
            cmd.Parameters.AddWithValue("@tf", from.Value.TimeOfDay);
            cmd.Parameters.AddWithValue("@tt", from.Value.TimeOfDay);
            ...
            else
            {
                ...
                cmd1.Parameters.AddWithValue("@docid", cardiac.doctor_id);
                cmd1.Parameters.AddWithValue("@timefrom", from.Value.TimeOfDay);
                cmd1.Parameters.AddWithValue("@timeto", from.Value.TimeOfDay);
